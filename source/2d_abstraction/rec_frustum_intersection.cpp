...
namespace rec {
	...
	*	If each frustumOne intersects frustumTwo it returns true, otherwise false.
	*/
	bool doMultipleFrustumsIntersect(std::vector<tree<rec::seAndFrust>::pre_order_iterator> frustumOneList, tree<rec::seAndFrust>::pre_order_iterator frustumTwo) {

		for (int i = 0; i < frustumOneList.size(); i++) {
			tree<rec::seAndFrust>::pre_order_iterator currentFrustumOne = frustumOneList.at(i);
			if (!doFrustumsIntesect(currentFrustumOne, frustumTwo)) {
				return false;
			}
		}
	...
	*/
	...
	std::vector<std::vector<tree<rec::seAndFrust>::pre_order_iterator>> intersectAllFrustums(std::vector<std::vector<tree<rec::seAndFrust>::pre_order_iterator>> cameraFrustumLists) {

		std::vector<std::vector<tree<rec::seAndFrust>::pre_order_iterator>> resultCombinations;

		//begin recursive computation with the frustumList of camera 0
		std::vector<tree<rec::seAndFrust>::pre_order_iterator> camera0FrustumList = cameraFrustumLists.at(0);

		for (int i = 0; i < camera0FrustumList.size(); i++) {
			tree<rec::seAndFrust>::pre_order_iterator currentFrustum = camera0FrustumList.at(i);

			//fList contains the frustums of the cameras already gathered and tested for intersection 
			std::vector<tree<rec::seAndFrust>::pre_order_iterator> fList;
			fList.push_back(currentFrustum);

			//start recursive computation
			std::vector<std::vector<tree<rec::seAndFrust>::pre_order_iterator>> resultCombinationsPart = testMultipleFrustumsWithMultipleFrustumsForIntersection(fList, cameraFrustumLists, 1);
	...
	*/
	...
	std::vector<std::vector<tree<rec::seAndFrust>::pre_order_iterator>> testMultipleFrustumsWithMultipleFrustumsForIntersection
		(std::vector<tree<rec::seAndFrust>::pre_order_iterator> fList, std::vector<std::vector<tree<rec::seAndFrust>::pre_order_iterator>> cameraFrustumLists, int currentList) {

		std::vector<std::vector<tree<rec::seAndFrust>::pre_order_iterator>> resultCombinations;

		//get cameraFrustumList which has to be processed in this recursion step
		std::vector<tree<rec::seAndFrust>::pre_order_iterator> currentCameraFrustumList = cameraFrustumLists.at(currentList);

		//process each frustum in currentCameraFrustumList
		for (int i = 0; i < currentCameraFrustumList.size(); i++) {
			tree<rec::seAndFrust>::pre_order_iterator currentFrustum = currentCameraFrustumList.at(i);

			//test if all already in fList gathered frustums intersect the currentFrustm
			if (doMultipleFrustumsIntersect(fList, currentFrustum)) {
				std::vector<tree<rec::seAndFrust>::pre_order_iterator> newfList = fList;
				newfList.push_back(currentFrustum);

				//this was not the last computation to be performed, there are cameraFrustumLists not evaluated yet
				if (currentList + 1 < cameraFrustumLists.size()) {
					//perform another recursive computation on the next cameraFrustumList
					std::vector<std::vector<tree<rec::seAndFrust>::pre_order_iterator>> resultCombinationsPart = testMultipleFrustumsWithMultipleFrustumsForIntersection(newfList, cameraFrustumLists, currentList + 1);
					//append received part of the result to the whole result
					resultCombinations.insert(resultCombinations.end(), resultCombinationsPart.begin(), resultCombinationsPart.end());
				}
