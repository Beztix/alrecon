...
double evaluateSuperellipseFitImplicid(vector<int> contourVector, double xc, double yc, double theta, double a, double b, double epsilon) {
...
double evaluateSuperellipseFitEuclidean(vector<int> contourVector, double xc, double yc, double theta, double a, double b, double epsilon) {
	int no_contourPixels = int(contourVector.size()) / 2;
	int* contourList = &contourVector[0];
	double cosine2 = cos(-theta);
	double sine2 = sin(-theta);
	...
	for (int i = 0; i < no_contourPixels; i++){
		...
		double dist = sqrt(pow((xRel - xs), 2.0) + pow((yRel - ys), 2.0));
...
vector<vector<double>> useRoninRecursive(int* pixelGrid, vector<int> contourVector, const int width, const int height, const int qualityValue) {
	...
	//evaluate the fit
	double currentValue = evaluateSuperellipseFitEuclidean(contourVector, xc, yc, theta, a, b, epsilon);
	...
	if (currentValue > qualityValue) {
		...
		for (int i = 0; i < blobsPart1.size(); i++) {
			...
			//threshold: if blob is too small don't fit to the blob (jsut ignore)
